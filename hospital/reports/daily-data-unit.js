...
dailyData = (doc) => {
    ...
    dataPipe = [{
        ...
        { "$match": { "$expr": { "$in": ["$hospitalId", "$hospitalGroupCms.hospitalid"] } } },
        {
            $lookup: {
                from: 'requestvil',
                "let": { "id": "$hospitalId" },
        ...
        },
        {
            $group: {
                ...
                refferalWiseVil: { $first: "$refferalWiseVil" },
                hospitalGroupCms: { $first: "$hospitalGroupCms" },
                hospitalQueryPatient: { $first: "$hospitalqueryRefferal" },
...

        {
            $group: {
                ...
                hospitalGroupCms: { $first: "$hospitalGroupCms" },
                hospitalQueryPatient: { $first: "$hospitalQueryPatient" },
        ...
        },
        {
            $group: {
                ...
                hospitalGroupCms: { $first: "$hospitalGroupCms" },
                hospitalQueryPatient: { $first: "$hospitalQueryPatient" },
                hospitalOwnQueryPatient: { $first: "$hospitalOwnQueryRefferal" },
        ...
        },
        {
            $group: {
                ...
                hospitalGroupCms: { $first: "$hospitalGroupCms" },
                hospitalQueryPatient: { $first: "$hospitalQueryPatient" },
                hospitalOwnQueryPatient: { $first: "$hospitalOwnQueryPatient" },
            }
        },
        {
            $unwind: "$hospitalOwnQueryRefferal"
...

        {
            $lookup: {
                ...
                as: 'facilitatorArrivalConfirmed',
        ...
        },
        {
            $lookup: {
                ...
                as: 'facilitatorArrivingSevenDays',
        ...
        },
        {
            $addFields: {
                arrivalConfirmed: {
                    $size: {
                        $concatArrays: [
                            "$hospitalArrivalConfirmed",
                            "$facilitatorArrivalConfirmed",
                ...
                arrivingToday: {
                    $size: {
                        $concatArrays: [
                            "$hospitalArrivingToday",
                            "$facilitatorArrivingToday",
                ...
                arrivingSevenDays: {
                    $size: {
                        $concatArrays: [
                            "$hospitalArrivingSevenDays",
                            "$facilitatorArrivingSevenDays",
...

        {
            $group: {
                _id: {
                    name: "$_id",
                    patient: "$hospitalQueryPatient.patient",

                },
                hospitalQueryPatient: { $push: "$hospitalQueryPatient" },
                treatmentWise: { $first: "$treatmentWise" },
                countryWise: { $first: "$countryWise" },
                hospitalWise: { $first: "$hospitalWise" },
                refferalWise: { $first: "$refferalWise" },
                arrivalConfirmed: { $first: "$arrivalConfirmed" },
                arrivingToday: { $first: "$arrivingToday" },
                arrivingSevenDays: { $first: "$arrivingSevenDays" },
                hospitalOwnQueryPatient: { $first: "$hospitalOwnQueryPatient" }
        ...
        {
            $unwind: "$hospitalOwnQueryPatient"
        },

        {
            $group: {
                _id: {
                    name: "$_id",
                    patient: "$hospitalOwnQueryPatient.patient",

                },
                hospitalOwnQueryPatient: { $push: "$hospitalOwnQueryPatient" },
                treatmentWise: { $first: "$treatmentWise" },
                countryWise: { $first: "$countryWise" },
                hospitalWise: { $first: "$hospitalWise" },
                refferalWise: { $first: "$refferalWise" },
                ...
                arrivingToday: { $first: "$arrivingToday" },
                arrivingSevenDays: { $first: "$arrivingSevenDays" },
                hospitalPatient: { $first: "$hospitalPatient" }
        ...
        },
        {
            $lookup: {
                from: 'patient',
                localField: 'hospitalPatient.value',
                foreignField: '_id',
                as: 'hospitalPatient',
        ...
        },
        {
            $lookup: {
                from: 'hospitalpatient',
                localField: 'hospitalOwnPatient.value',
                foreignField: '_id',
                as: 'hospitalOwnPatient',

            }
        },
        {
            $project: {
                "hospitalOwnPatient.name": 1,
                "hospitalOwnPatient.country": 1,
                "hospitalOwnPatient.treatment": 1,
                "hospitalPatient.name": 1,
                "hospitalPatient.country": 1,
                "hospitalPatient.treatment": 1,
                treatmentWise: 1,
                countryWise: 1,
                hospitalWise: 1,
                refferalWise: 1,
